...
class App extends React.Component {
  ...
  render() {
    return (
      <div style={{fontFamily: 'Arial, sans-serif'}} className="container-fluid col-lg-9 col-md-11 col-sm-9">
        <div ref={this.header} >
          <Header/>
        </div>
        <Navbar ref={this.menu} expand="lg" bg="dark" variant="dark" className="rounded">
          <Navbar.Brand onClick={() => this.scrollToElement('header')}>Curriculum Vitae</Navbar.Brand>
          <Navbar.Toggle aria-controls="basic-navbar-nav" />
          <Navbar.Collapse id="basic-navbar-nav">
            <Nav className="mr-auto">
              <Nav.Link onClick={() => this.scrollToElement('education')}>Education</Nav.Link>
              <Nav.Link onClick={() => this.scrollToElement('experience')}>Professional experience</Nav.Link>
